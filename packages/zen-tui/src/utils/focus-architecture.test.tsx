...
import { signal, createRoot, computed, setPlatformOps } from '@zen/runtime';
import { tuiPlatformOps } from '../core/platform-ops.js';
import { dispatchInput, clearInputHandlers, useInput } from '../hooks/useInput.js';
    ...
    createRoot(() => {
      return FocusProvider({
        get children() {
          ...
          useInput(
            (input) => { log.push(`A: "${input}"`); return false; },
            { isActive: focusAResult.isFocused }
          );
          
          focusBResult = useFocus({ id: 'item-b' });
          useInput(
            (input) => { log.push(`B: "${input}"`); return false; },
...
describe('AllTest-like structure (manual simulation)', () => {
  test('List (autoFocus) + TextArea with gate', async () => {
    ...
    console.log('  List focused:', listFocusResult?.isFocused?.value);
    console.log('  TextArea focused:', textareaFocusResult?.isFocused?.value);
    ...
    log.length = 0;
    dispatchInput('a');
    console.log('After typing "a":', log);
    ...
    log.length = 0;
    dispatchInput('b');
    console.log('After typing "b":', log);
...
describe('AllTest with gate starting closed', () => {
  test('Gate closed -> open -> Tab navigation', async () => {
    ...
    // Initially gate is closed (header mode)
    console.log('Initial state (header mode):');
    console.log('  Gate:', scopeActive.value);
    console.log('  List FP focus:', listFocusResult?.isFocused?.value);
    console.log('  TextArea FP focus:', textareaFocusResult?.isFocused?.value);

    // With gate closed, no component should receive input
    log.length = 0;
    dispatchInput('x');
    console.log('Input in header mode:', log);
    ...
    console.log('After entering content mode:');
    console.log('  Gate:', scopeActive.value);
    console.log('  List FP focus:', listFocusResult?.isFocused?.value);
    console.log('  TextArea FP focus:', textareaFocusResult?.isFocused?.value);
    ...
    log.length = 0;
    dispatchInput('c');
    console.log('After Tab back, typing "c":', log);
